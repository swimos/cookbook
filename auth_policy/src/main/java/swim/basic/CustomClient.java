...
/**
 * The complimentary code as part of the <a href="https://swimos.org/tutorials/auth-policy/">Auth
 * Policy</a> cookbook.
 *
 * <p>In this cookbook, a plane is created with an auth policy. Requests to the server will only be
 * accepted if the token provided has sufficient permissions.
 *
...
public final class CustomClient {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    swimClient.command(
        hostUri + "?token=aaa", nodeUri, laneUri, Text.from("Command with invalid token"));
    swimClient.command(
        hostUri + "?token=abc", nodeUri, laneUri, Text.from("Command with user token"));
    swimClient.command(
        hostUri + "?token=abc123", nodeUri, laneUri, Text.from("Command with admin token"));
    ...
    final ValueDownlink<Value> adminInfoDownlink =
        swimClient
            .downlinkValue()
            ...
            .didLink(
                () ->
                    System.out.println(
                        "link to adminInfo successful with no token")) // This will not get printed
            // as a token is required
            .open();
    ...
    final ValueDownlink<Value> adminInfoDownlinkWithToken =
        swimClient
            .downlinkValue()
            .valueForm(Form.forValue())
