...
public class SupplierAgent extends AbstractAgent {
  ...
  @SwimLane("register")
  CommandLane<String> register = this.<String>commandLane().onCommand(this::addResupplyDownlink);

  /**
   * Creates a downlink to a given location that will resupply stock when it falls below the
   * threshold
   */
        ...
        .didUpdate(
            (item, newValue, oldValue) -> {
              if (newValue < RESUPPLY_THRESHOLD) { // If the stock is too low then resupply it
                ...
                this.command(
                    SupplierPlane.WAREHOUSE_HOST_URI,
                    warehouseNodeUri,
                    "resupply",
                    Text.from(item));
              }
